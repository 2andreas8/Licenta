...
def generate_summary_for_chunks_with_cancellation(chunks, max_length=5000, document_title=None, task_id=None):
    """Versiune care verifică anularea pentru generate_summary_for_chunks."""
    from nlp.routes import active_summary_tasks  # local import to avoid circular dependency
    
    if task_id:
        active_summary_tasks[task_id]["start_time"] = time.time()
    
    if len(chunks) <= 3:
        if task_id and active_summary_tasks.get(task_id, {}).get("canceled", False):
            print(f"Summary task {task_id} was canceled")
            return None
        combined_text = "\n\n".join(chunks)
        return generate_summary(combined_text)
    ...
    for chunk in chunks:
        if task_id and active_summary_tasks.get(task_id, {}).get("canceled", False):
            print(f"Summary task {task_id} was canceled during deduplication")
            return None
            
        chunk_hash = hashlib.md5((chunk[:50] + chunk[-50:]).encode('utf-8')).hexdigest()
        if chunk_hash not in content_hashes:
            content_hashes.add(chunk_hash)
            unique_chunks.append(chunk)
    ...
    for chunk in unique_chunks:
        if task_id and active_summary_tasks.get(task_id, {}).get("canceled", False):
            print(f"Summary task {task_id} was canceled during grouping")
            return None
    ...
    for i, group in enumerate(groups):
        ...
        if task_id and active_summary_tasks.get(task_id, {}).get("canceled", False):
            print(f"Summary task {task_id} was canceled during group {i+1} processing")
            return None
    ...
    # checks for task cancellation
    if task_id and active_summary_tasks.get(task_id, {}).get("canceled", False):
        print(f"Summary task {task_id} was canceled before final summary")
        return None
